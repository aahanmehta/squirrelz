...
import { abbrState } from "./stateNameChanger.js";

//XMLHttpRequest for getting specific state data
...
function showSpecificGraph(e) {
    ...
    var name = abbrState(e.target.feature.properties.name, 'abbr');
    //console.log(name);
    getStateData(name);
...
function state_ufo(data) {
  ...
  var mousemove = function(d) {
   tooltip
     .html(d.Year + ": " + d.sightings + " UFO sightings " + d.ethanol_per_capita + " gallons of ethanol consumed per capita")
     .style("left", d3.event.pageX + "px") // It is important to put the +90: other wise the tooltip is exactly where the point is an it creates a weird effect
     .style("top", d3.event.pageY + "px")
  }
